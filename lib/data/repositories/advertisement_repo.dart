...
class AdvertisementRepository {
  ...
  Future<ListPage<Advertisement>> getAll(
      {int pageNumber = 1, required int size, String? category}) async {
    return await apiCall.getAll(
        number: pageNumber, size: size, category: category);
  }

  Future<ListPage<Advertisement>> search(
      {int pageNumber = 1, int size = 10, required String searchInput}) async {
    return await apiCall.search(searchInput: searchInput, number: pageNumber);
  }
